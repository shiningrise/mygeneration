...
namespace Zeus.UserInterface
{
    ...
    public class GuiComboBox : GuiControl, IGuiComboBox, IGuiBindableListControl
	{
		...
		{
			get { return this._items[val]; }
			set { this._items[val] = value; }
		}
		...
		public string SelectedText 
		{
			get
			{
				return this._items[this._value];
			}
		}
		...
		public bool IsSelectedAtIndex(int index) 
		{
			...
			if ((index < this.Count) && (index >= 0))
				isselected = (GetValueAtIndex(index) == SelectedValue);
		...
		public override string[] GetEventHandlers(string eventType)
		{
			if (eventType == "onchange") 
			{
				return this._onclickEvents.ToArray();
			}
			else
			{
